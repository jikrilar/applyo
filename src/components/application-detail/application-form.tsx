...
export function ApplicationForm({ stages, currency, application, onSaved, onCancel }: Props) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const companyVal = text(data, "company").trim();
    const positionVal = text(data, "position").trim();
    if (!companyVal) nextFieldErrors.company = "Perusahaan wajib diisi.";
    else if (companyVal.length > 200) nextFieldErrors.company = "Perusahaan maksimal 200 karakter.";
    if (!positionVal) nextFieldErrors.position = "Posisi wajib diisi.";
    else if (positionVal.length > 200) nextFieldErrors.position = "Posisi maksimal 200 karakter.";
    ...
    startTransition(async () => {
      ...
      setDirty(false); setError(""); setFieldErrors({}); onSaved?.(result.data); router.refresh();
    });
  };

  return <form className="application-full-form" onSubmit={submit} onChange={() => setDirty(true)} noValidate>
    <fieldset><legend>Informasi utama</legend><div className="application-form-grid">
      <label className="field-group"><span>Perusahaan *</span><span className="field-control"><input name="company" defaultValue={application?.company} maxLength={200} required aria-required="true" aria-invalid={Boolean(fieldErrors.company)} aria-describedby={fieldErrors.company ? "company-error" : undefined} /></span>{fieldErrors.company && <span id="company-error" className="field-error" role="alert">{fieldErrors.company}</span>}</label>
      <label className="field-group"><span>Posisi *</span><span className="field-control"><input name="position" defaultValue={application?.position} maxLength={200} required aria-required="true" aria-invalid={Boolean(fieldErrors.position)} aria-describedby={fieldErrors.position ? "position-error" : undefined} /></span>{fieldErrors.position && <span id="position-error" className="field-error" role="alert">{fieldErrors.position}</span>}</label>
