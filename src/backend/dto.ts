...
export type ApplicationListItemDTO = ApplicationCardDTO & { archivedAt: string | null; closedAt: string | null };
...
export type DashboardSummaryDTO = { total: number; active: number; interviews: number; assessments: number; offers: number; upcoming: CalendarEventDTO[]; needsAttention: NeedsAttentionDTO[] };
...
export function toStageDTO(row: PipelineStageRow): StageDTO { return { id: row.id, name: row.name, systemKey: row.system_key, colorKey: row.color_key, position: row.position, isClosed: row.is_closed, isVisible: row.is_visible }; }
